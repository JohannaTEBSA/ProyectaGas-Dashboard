    ...
    with tab4:
        ...
        with col2:
            ...
            if datos['metricas_resumen'] is not None:
                ...
                # Mostrar métricas como cards
                for idx, row in df_resumen.iterrows():
                    variable = row['Variable']
                    mape = row['MAPE']
                    r2 = row['R²']
                    
                    # Determinar color según MAPE
                    if mape < 3:
                        color = "#28A745"  # Verde
                    ...
                    <div style='background: linear-gradient(135deg, {color}20 0%, {color}40 100%); 
                        ...
                        <h4 style='margin: 0; color: {color};'>{variable.replace('_', ' ')}</h4>
                        <div style='display: flex; justify-content: space-between; margin-top: 10px;'>
                            <div>
                                <p style='margin: 0; font-size: 0.9em; color: #666;'>MAPE</p>
                                <p style='margin: 0; font-size: 1.5em; font-weight: bold;'>{mape:.2f}%</p>
                            </div>
                            <div>
                                <p style='margin: 0; font-size: 0.9em; color: #666;'>R²</p>
                                <p style='margin: 0; font-size: 1.5em; font-weight: bold;'>{r2:.3f}</p>
                            </div>
                        </div>
                        <p style='margin: 5px 0 0 0; font-size: 0.9em; color: {color}; font-weight: bold;'>
                            {calificacion}
                        </p>
                    </div>
                    """, unsafe_allow_html=True)
